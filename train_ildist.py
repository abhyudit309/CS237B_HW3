...
class NN(tf.keras.Model):
    def __init__(self, in_size, out_size):
        ...
        output_size = out_size ** 2 + out_size
        ...
        hidden_layer2 = tf.keras.layers.Dense(45, activation='tanh', kernel_initializer=tf.keras.initializers.GlorotUniform())(hidden_layer1)
        output = tf.keras.layers.Dense(output_size)(hidden_layer2)
...
def loss(y_est, y):
    ...
    epsilon = 0.001
    N, action_dim = y.shape
    mu = y_est[:, 0:action_dim]
    A = tf.reshape(y_est[:, action_dim:], (N, action_dim, action_dim))
    cov = tf.matmul(A, tf.transpose(A, perm=[0, 2, 1])) + epsilon * tf.eye(action_dim)
    mvn = tfd.MultivariateNormalTriL(loc=mu, scale_tril=tf.linalg.cholesky(cov))
    l = tf.reduce_sum(mvn.log_prob(y))
    return -l / N
