...
if __name__ == '__main__':
    ...
    for _ in range(10):
        ...
        while not done:
            ...
            G = len(goals[scenario_name])  # number of goals
            a_robot = np.zeros((G, 2))
            for i in range(G):
                goal = goals[scenario_name][i]
                y_est = nn_models[goal](obs).numpy().ravel()
                mu = y_est[0:2]
                a_robot[i] = Pg[i] * (optimal_action[goal].ravel() - mu)
            a_robot = np.sum(a_robot, axis=0)
            a_robot[0] = np.clip(a_robot[0], -max_steering, max_steering)
            a_robot[1] = np.clip(a_robot[1], -max_throttle, max_throttle)
            a_robot = a_robot[np.newaxis, :]
            ...
            for i in range(G):
                ...
                y_est = nn_models[goal](obs).numpy().ravel()
                mu = y_est[0:2]
                A = np.reshape(y_est[2:], (2, 2))
                cov = np.matmul(A, np.transpose(A)) + epsilon * np.eye(2)
                probs[i] = multivariate_normal.pdf(a_human, mean=mu, cov=cov)
