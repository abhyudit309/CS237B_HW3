    ...
    for _ in range(15):
        ...
        while not done:
            ...
            for i in range(G):
                ...
                A = np.reshape(y_est[action_dim:], (action_dim, action_dim))
                cov = np.matmul(A, np.transpose(A)) + epsilon * np.eye(action_dim)
                probs[i] = multivariate_normal.pdf(action, mean=mu, cov=cov)
