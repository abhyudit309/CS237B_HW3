...
class NN(tf.keras.Model):
    def __init__(self, in_size, out_size):
        ...
        hidden_layer1 = tf.keras.layers.Dense(90, activation='tanh', kernel_initializer=tf.keras.initializers.GlorotUniform())(input_layer)
        hidden_layer2 = tf.keras.layers.Dense(60, activation='tanh', kernel_initializer=tf.keras.initializers.GlorotUniform())(hidden_layer1)
        hidden_layer3 = tf.keras.layers.Dense(30, activation='tanh', kernel_initializer=tf.keras.initializers.GlorotUniform())(hidden_layer2)
        self.base_model = tf.keras.Model(inputs=input_layer, outputs=hidden_layer3)

        self.output1 = tf.keras.layers.Dense(out_size)
        self.output2 = tf.keras.layers.Dense(out_size)
        self.output3 = tf.keras.layers.Dense(out_size)
    ...
    def call(self, x, u):
        ...
        ind1 = tf.cast(tf.math.equal(u, 0), tf.float32)
        ind2 = tf.cast(tf.math.equal(u, 1), tf.float32)
        ind3 = tf.cast(tf.math.equal(u, 2), tf.float32)

        z = self.base_model(x)
        z1 = tf.multiply(z, ind1)
        z2 = tf.multiply(z, ind2)
        z3 = tf.multiply(z, ind3)

        y1 = tf.multiply(self.output1(z1), ind1)
        y2 = tf.multiply(self.output2(z2), ind2)
        y3 = tf.multiply(self.output3(z3), ind3)
...
def loss(y_est, y):
    ...
    alpha = 0.99  # penalizing factor
    steering_loss = (y_est[:, 0] - y[:, 0]) ** 2
    throttle_loss = (y_est[:, 1] - y[:, 1]) ** 2
    overall_loss = alpha * steering_loss + (1 - alpha) * throttle_loss
    return tf.reduce_mean(overall_loss)
...
def nn(data, args):
    ...
    def train_step(x, y, u):
        ...
        grad = g.gradient(current_loss, nn_model.trainable_variables)
